...
function navClass({ isActive }) {
  return `flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${
    isActive
...
export default function Navbar() {
  ...
  function scrollNav(dir) {
    navScrollRef.current?.scrollBy({ left: dir * 140, behavior: 'smooth' });
  }
  ...
  // Track nav scroll arrows
  useEffect(() => {
    const el = navScrollRef.current;
    if (!el) return;
    // Delay initial check so DOM is fully painted
    const t = setTimeout(updateScrollState, 50);
    el.addEventListener('scroll', updateScrollState);
    window.addEventListener('resize', updateScrollState);
    // ResizeObserver catches content width changes
  ...
  const NavLinks = ({ mobile = false }) => {
    ...
    return (
      <>
        ...
        {hasRole('encargado', 'cajero') && (
          <NavLink to="/gastos-sucursal" className={cls} onClick={close}>
        ...
        {hasRole('dueno') && (
          <NavLink to="/payment-methods" className={cls} onClick={close}>
            {({ isActive }) => (<><IconPaymentMethods active={isActive} />Pagos</>)}
          </NavLink>
        )}
        ...
        {hasRole('cajero', 'encargado', 'dueno') && (
          <NavLink to="/fiscal" className={cls} onClick={close}>
  ...
  return (
    <nav className="bg-gradient-to-r from-blue-900 to-blue-700 border-b border-blue-800 sticky top-0 z-10 shadow-md">
      <div className="px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between">
        <div className="flex items-center gap-3 sm:gap-5 min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-shrink-0">
            <CoreLogo size={30} />
            <span className="font-bold text-white text-xl tracking-tight">Core</span>
          </div>
          {/* Desktop nav links */}
          <div className="hidden lg:flex items-center relative flex-1 min-w-0">
            {canScrollLeft && (
              <button
                onClick={() => scrollNav(-1)}
                className="absolute left-0 z-10 h-full pl-0.5 pr-2 flex items-center bg-gradient-to-r from-blue-900 via-blue-900/80 to-transparent text-white/80 hover:text-white transition-colors"
              >
            ...
            <div
              ...
              className="flex items-center gap-0.5 overflow-x-auto no-scrollbar w-full"
            >
              <NavLinks />
            </div>
            {canScrollRight && (
              <button
                onClick={() => scrollNav(1)}
                className="absolute right-0 z-10 h-full pr-0.5 pl-2 flex items-center bg-gradient-to-l from-blue-900 via-blue-900/80 to-transparent text-white/80 hover:text-white transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden lg:block relative" ref={profileRef}>
            ...
            >
              <div className="text-xs leading-tight text-left">
                ...
                <div className="text-indigo-200">{ROLE_LABELS[user?.role] ?? user?.role}</div>
              ...
              <svg className={`w-3 h-3 text-indigo-300 transition-transform flex-shrink-0 ${profileOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            className="lg:hidden flex flex-col justify-center items-center w-9 h-9 rounded-lg hover:bg-white/10 transition-colors gap-1.5"
            aria-label="Menú"
          >
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="lg:hidden border-t border-blue-800 bg-blue-900/95 px-4 py-3 space-y-1">
          <NavLinks mobile />
